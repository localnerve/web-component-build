...
class WebComponentBuild {
  ...
  constructor (outputDir, jsPath, cssPath, htmlPath, minifySkip = false) {
    ...
    if (jsPath) {
      this.outputJsFile = path.join(
        this.outputDir,
        path.basename(this.inputJsFile)
      );
    }
    if (htmlPath) {
      this.outputHtmlFile = path.join(
        this.outputDir, path.basename(this.inputHtmlFile)
      );
    }
    if (cssPath) {
      this.outputCssFile = path.join(
        this.outputDir, path.basename(this.inputCssFile)
      );
    }
  }
  ...
  async minifyCss () {
    ...
    const cleanCss =
      this.minifySkip ? cssText : new CleanCss().minify(cssText).styles;
  ...
  async minifyHtml (htmlText) {
    const minifiedHtml = this.minifySkip ? htmlText : _minifyHtml(htmlText, {
      minifyJS: true,
      minifyCSS: true,
   ...
   * @param {Object} terserOptions - terser minification options.
   * @returns {String} minified js.
   */
  async minifyJs (jsText, terserOptions) {
    const minifiedJs =
      this.minifySkip ? jsText : (await _minifyJs(jsText, terserOptions)).code;
   ...
   */
  get output () {
    const cssPath = this.outputCssFile;
    const htmlPath = this.outputHtmlFile;
    const jsPath = this.outputJsFile;

    return {
      async getCssText () {
        let cssText;
        try {
          cssText = await fs.readFile(cssPath, {
            encoding: 'utf8'
          });
        }
      ...
      async getHtmlText () {
        let htmlText;
        try {
          htmlText = await fs.readFile(htmlPath, {
            encoding: 'utf8'
          });
        }
        catch {} // eslint-disable-line
      ...
      async getJsText () {
        let jsText;
        try {
          jsText = await fs.readFile(jsPath, {
            encoding: 'utf8'
          });
        }
        catch {} // eslint-disable-line
