 ...
 *
 * @param {String} jsPath - full path to the javascript file for the web component
 ...
 * @param {Boolean} [options.minifySkip] - flag to skip all minification (debug)
 * @param {String} [options.terserOptions] - Js minifier options (terser)
 */
export async function build (jsPath, outputDir, {
  cssPath,
  cssLinkHref,
  htmlPath,
  jsReplacement,
  ...
  let jsText = await fs.readFile(jsPath, { encoding: 'utf8' });
  let cssText, htmlText;

  if (cssPath) {
    cssText = await build.minifyCss();
  }
  ...
  if (htmlPath) {
    ...
    htmlText = await build.minifyHtml($('body').html());
  }
  ...
  build.minifyJs(jsText, terserOptions);
  return build.output;
}